...
4. Verify the Ed25519 signature against the cached public key. Sub millisecond on any modern CPU.
   - Key eligibility. The key a signature is verified against MUST have been resolved (step 3) for an authority the verifier trusts for this credential: the authority named by `issuerDid`, or, under `atcVersion` 1.1 only, an authority that appears in the signed `issuerChain` and is also in the verifier's trusted-issuer set. A DID that appears in `issuerChain` but is not a trusted issuer contributes no eligible key: the chain is signed by the very key whose eligibility is in question, so it cannot be its own authority (§13, Delegation and chain abuse). Under `atcVersion` 1.0 `issuerChain` is unsigned (§1.3a.1) and contributes no eligible key. A signature that verifies only against some other configured key is a signature failure (`SIGNATURE_INVALID`), not an issuer failure.
